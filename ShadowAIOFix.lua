...
---------INFO / Shadow read pls ------------
--[[

All fixes from me:
	
	
	- you have used many times ,,Control.KeyDown,, but never used Control.KeyUp ( if only KeyDown then the key is never Up and cant cast again)
	
	
	- for gamsteron Prediction, i have removed all ,,COLLISION_YASUOWALL,, ( this check particleAPI in predictionLib and crash/freeze GoS) Api must fix from Fere!
	
	
	- for gamsteron Prediction, you have many times checked COLLISION_MINION and COLLISION_ENEMYHERO with MaxCollision(1)....
		the result from this pred.check = if one minion collision then cast //// if one Enemy collision then cast ( dont use COLLISION_ENEMYHERO )
		you can use this for example Lux Q ,,, the Q can stun 2 targets ( 1Minion+1Enemy or 2 Enemys )
		
		
	-- TIP --
	If you Copy and Paste your champ code for a new champ then check more times ( you had some wronge stuff ) ;) 
]]
