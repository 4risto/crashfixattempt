...
function Morgana:__init()
    self.QData = {Type = _G.SPELLTYPE_LINE, Delay = 0.25, Radius = 70, Range = 1175, Speed = 1200, Collision = true, MaxCollision = 0, CollisionTypes = {_G.COLLISION_MINION}}
...
function KogMaw:__init()
    self.QData = {Delay = 0.25, Radius = 70, Range = 1175, Speed = 1650, Type = _G.SPELLTYPE_LINE, Collision = true, MaxCollision = 0, CollisionTypes = {_G.COLLISION_MINION}}
...
function Brand:__init()
    self.ETarget = nil
    self.QData = {Delay = 0.25, Radius = 60, Range = 1085, Speed = 1600, Type = _G.SPELLTYPE_LINE, Collision = true}
...
function Brand:Tick()
    ...
    if SDKSpell:IsReady(_Q, {q = 0.5, w = 0.53, e = 0.53, r = 0.33}) then
        ...
        -- Combo Harass  
        if (SDKOrbwalker.Modes[ORBWALKER_MODE_COMBO] and Menu.qset.comhar.combo:Value()) or (SDKOrbwalker.Modes[ORBWALKER_MODE_HARASS] and Menu.qset.comhar.harass:Value()) then
            if Game.Timer() < SDKSpell.EkTimer + 1 and Game.Timer() > SDKSpell.ETimer + 0.33 and AIO:IsValidHero(self.ETarget) and self.ETarget:GetCollision(self.QData.Radius, self.QData.Speed, self.QData.Delay, collisionTypes (table {0})) == 0 then
            ...
            for i = 1, #enemyList do
                ...
                if SDKBuff:GetBuffDuration(unit, "brandablaze") > 0.5 and unit:GetCollision(self.QData.Radius, self.QData.Speed, self.QData.Delay, collisionTypes (table {0})) == 0 then
        ...
        elseif Menu.qset.auto.stun:Value() then
            if Game.Timer() < SDKSpell.EkTimer + 1 and Game.Timer() < SDKSpell.ETimer + 1 and AIO:IsValidHero(self.ETarget) and self.ETarget:GetCollision(self.QData.Radius, self.QData.Speed, self.QData.Delay, collisionTypes (table {0})) == 0 then
            ...
            for i = 1, #enemyList do
                local unit = enemyList[i]
                if unit and SDKBuff:GetBuffDuration(unit, "brandablaze") > 0.5 and unit:GetCollision(self.QData.Radius, self.QData.Speed, self.QData.Delay, collisionTypes (table {0})) == 0 then
                    blazeList[#blazeList + 1] = unit
                end
            end
        ...
        elseif myHero:GetSpellData(_Q).level > 0 and myHero:GetSpellData(_W).level > 0 then
            ...
            if Menu.eset.auto.stun:Value() and myHero.mana > myHero:GetSpellData(_Q).mana + myHero:GetSpellData(_E).mana then
                if (Game.CanUseSpell(_Q) == 0 or myHero:GetSpellData(_Q).currentCd < 0.75) and not(Game.CanUseSpell(_W) == 0 or myHero:GetSpellData(_W).currentCd < 0.75) then
                    ...
                    local eTarget = SDKTarget:GetTarget(blazeList, 1)
                    if eTarget and eTarget:GetCollision(self.QData.Radius, self.QData.Speed, self.QData.Delay, collisionTypes (table {0})) == 0 and AIO:Cast(HK_E, eTarget) then
                        return
                    end
                    if Game.Timer() > SDKSpell.QkTimer + 0.77 and Game.Timer() > SDKSpell.WkTimer + 1.33 and Game.Timer() > SDKSpell.RkTimer + 0.77 then
                        eTarget = SDKTarget:GetTarget(enemyList, 1)
...
function Ezreal:__init()
    self.QData = {Delay = 0.25, Radius = 60, Range = 1150, Speed = 2000, Type = _G.SPELLTYPE_LINE, Collision = true, MaxCollision = 0, CollisionTypes = {_G.COLLISION_MINION}}
